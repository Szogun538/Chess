...
func add_history(start, end):
	...
	var move = Move.new(start, end, type, Move.PieceType.B)
...
func moves(posi: Vector2, mode: int):
	...
	var loop_tile = posi
	var direction: Array[Vector2] = [Vector2(1,1), Vector2(1,-1), Vector2(-1,-1), Vector2(-1,1)]
	for dir in direction:
		while loop_tile.x < 7 and loop_tile.y < 7 and dir == Vector2(1,1) or loop_tile.x < 7 and loop_tile.y > 0 and dir == Vector2(1,-1) or loop_tile.x > 0 and loop_tile.y > 0 and dir == Vector2(-1,-1) or loop_tile.x > 0 and loop_tile.y < 7 and dir == Vector2(-1,1):
			loop_tile += dir
			if mode <= 2:
				if mode == 0:
				...
				if table.tile_base_on_position(loop_tile).piece_standing != null:
					...
					if table.tile_base_on_position(loop_tile).piece_standing.name == "king_w":
						...
						for i in scan_tiles:
							...
							elif i.piece_standing.name != "king_w" and i.piece_standing.is_white != is_white:
